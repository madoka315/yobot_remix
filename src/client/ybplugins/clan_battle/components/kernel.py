...
def execute(self, match_num, ctx):
    ...
    if match_num == 1:  # 创建
        ...
        #########
        match = re.match(r'^业绩(表) *$', cmd) or re.match(r'^查刀$', cmd)
        if not match: return
        try:
    ...
    ###########
        ...
        try:
            ...
            for i in knifes:
                flag = ''
                if i['health_remain'] == 0 and i['is_continue'] == False:
                    num += 0.5
                    flag = '(尾刀)'
    ...
    # elif match_num == 11:  # 挂树
        ...
        ###########
        match = re.match(r'^(?:打了|报伤害|更新出刀)*(?::|：)?(?:剩| |)(?:(\d+(?:s|S|秒))?(?:打了| |)(\d+)(?:w|W|万| |))? *(.*?)?(?:\[CQ:at,qq=(\d+)\])? *$', cmd)
        if not match: return '格式出错(O×O)\n如“更新出刀：200w 不想出”\n“报伤害 2s200w”\n“报伤害 3s300w@xxx”'
        s = match.group(1) or 1
        if s != 1: s = re.sub(r'([a-z]|[A-Z]|秒)', '', s)
        hurt = (match.group(2) and int(match.group(2))) or 0
        challenge_comment = match.group(3)
        if challenge_comment == '':
            challenge_comment = None  
        behalf = match.group(4) and int(match.group(4))
        if behalf: user_id = behalf
        if not self.check_blade(group_id, user_id):
            return '你都没申请出刀，报啥子伤害啊 (╯‵□′)╯︵┻━┻'   
        return self.report_hurt(int(s), hurt, group_id, user_id, 0, challenge_comment)
