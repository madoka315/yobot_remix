...
#记录伤害/清空伤害
################
def report_hurt(self, s, hurt, group_id:Groupid, qqid:QQid, clean_type = 0, challenge_comment=None):
    """
    记录/清空出刀暂停后，成员报的伤害
    ...
    if clean_type == 0:
        ...
        if challenge_comment:
            challenging_member_list[boss_num][str_qqid]['challenge_comment'] = challenge_comment
        ...
        if challenging_member_list[boss_num][str_qqid]['damage'] == 0:
            ...
            if 'challenge_comment' in challenging_member_list[boss_num][str_qqid]:
                challenging_member_list[boss_num][str_qqid].pop('challenge_comment')
            ret_msg = '取消成功~'
    ...
    if challenging_list:
        ...
        for challenger, info in challenging_list.items():
            ...
            if info['behalf']:
                ...
                ###########
            if 11000 >= info['damage'] > 0:
                temp_msg += f', 剩{info["s"]}秒，打了{info["damage"]}万伤害'
            elif info['damage'] > 11000:
                temp_msg += f', 剩{info["s"]}秒，打了{info["damage"]}伤害'
            if 'challenge_comment' in info:
                temp_msg += f'({info["challenge_comment"]})'
                ########
            if info['tree']:
                temp_msg += ', 已挂树'
            msg.append(temp_msg)
        ...
        if challenging_list and boss_num_str in challenging_list:
            for challenger, info in challenging_list[boss_num_str].items():
                ...
                if 11000 > info['damage'] > 0:
                    ...
                    challenger_msg += f'@{info["s"]}s,{info["damage"]}'
                if 'challenge_comment' in info:
                    challenger_msg += f'({info["challenge_comment"]})'    
                    ###################    
                if info['tree']:
                    challenger_msg += '(挂树)'
